...
# 初始化模型
# model = AE_LSTM(input_dim=X_train.shape[2],hidden_dim=128,max_len=10,loss_type=loss_type,beta=beta,gamma=gamma).to(device)

# model = AE_ATT_LSTM(input_dim=X_train.shape[2],hidden_dim=128,max_len=10,loss_type=loss_type,beta=beta,gamma=gamma).to(device)
# model = AE_ATT_LightConv(input_dim=X_train.shape[2],hidden_dim=hidden_dim,max_len=10,loss_type=loss_type,beta=beta,gamma=gamma).to(device)
# model = AE_ATT_TCN(input_dim=X_train.shape[2],hidden_dim=hidden_dim,max_len=10,loss_type=loss_type,beta=beta,gamma=gamma).to(device)

# model = AE_ATT_LightTCN(input_dim=X_train.shape[2],hidden_dim=hidden_dim,max_len=10,loss_type=loss_type,beta=beta,gamma=gamma).to(device)

model = AE_ATT_BiLightTCN(input_dim=X_train.shape[2],hidden_dim=hidden_dim,max_len=10,loss_type=loss_type,beta=beta,gamma=gamma).to(device)
...
# 输出预测结果
y,y_pred = predict(model, test_iter)
